...
class Client(object):
    ...
    * popular songs
	...
	stream = self.getStreamBySongID(song_id)
	result = self.getSongByToken(stream['FileToken'])
	return Song.from_response(result, self.connection)

    def getStreamBySongID(self, song_id):
        result = self.connection.request('getStreamKeyFromSongIDEx', {'songID' :song_id, 'country' : self.connection.session.country,                                                                           'prefetch' : False, 'mobile' : False},                                              self.connection.header('getStreamKeyFromSongIDEx', 'jsqueue'))[1]
	return result

    def getSongByToken(self, song_token):
	result = self.connection.request('getSongFromToken', {'token' :song_token, 'country' : self.connection.session.country,},                                              self.connection.header('getSongFromToken'))[1]
	return result
